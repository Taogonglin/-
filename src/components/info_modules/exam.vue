<template>
    ...
    <div class="center">
      <!--头部-->
      <login-head/>
       <el-page-header @back="goBack" content="考试安排"></el-page-header>
        <!-- <div style="margin-top: 20px;display: flex;justify-content: center">
            <el-input v-model="keywords" placeholder="通过考试科目名称搜索..." prefix-icon="el-icon-search"
            ...
            <el-card class="hr-card" v-for="(user,index) in users" :key="index">
                ...
                <div>
                    <div class="img-container">
                        <img :src="avatar" :alt="user.name" :title="user.name" class="userface-img">
                    </div>
                    <div class="userinfo-container">
                        <div>课程代码：{{user.cid}}</div>
                        <div>考试科目：{{user.name}}</div>
                        <div>考试日期：{{user.date}}</div>
                        <div>考试安排：{{user.time}}</div>
                        <div>考试地点：{{user.location}}</div>
                        <div>座位号：{{user.seatNum}}</div>
                        <div>状态：{{user.remark}}</div>
                        <!-- <div>用户角色：
...
<script>
    export default {
        ...
        data() {
            return {
                ...
                total: 0,
                avatar: '../../../static/img/SCUlogolo.png', 
                users: [],
                allroles: [],
                selectedRoles: [],
                exceptRoles: [],
    ...
    methods: {
            ...
            let url = 'http://127.0.0.1:8080/exam/' + cookie_id;
